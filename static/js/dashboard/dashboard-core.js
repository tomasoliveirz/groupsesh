...
(function() {
    ...
    const DashboardCore = {
        ...
        async init(config = null) {
            ...
            try {
                ...
                // Extrair token da URL se não fornecido na config
                ...
                // Notificar que o dashboard está pronto
        ...
        _initComponents() {
            ...
            const calendarEl = GroupSesh.Utils.DOMUtils.getElementById('calendar');
            if (calendarEl) {
                GroupSesh.Dashboard.CalendarManager.init(calendarEl);
            }
            
            // Inicializar lista de participantes
            const participantsList = GroupSesh.Utils.DOMUtils.getElementById('participants-list');
            if (participantsList) {
        ...
        /**
         * Carrega dados da survey
         * @private
         */
        async _loadData() {
            try {
                // Usar informações básicas da configuração inicial se disponíveis
         ...
         * @param {Object} surveyConfig - Configuração da survey
         * @private
         */
         ...
         * @param {Object} surveyInfo - Informações da survey
         * @private
         */
        ...
        _ensureAdminIsParticipant() {
            if (!this._surveyData || !this._surveyData.survey || !this._surveyData.participants || 
                !this._surveyData.survey.admin_email || !this._surveyData.survey.admin_name) {
        ...
        /**
         * Atualiza a contagem de participantes na interface
         * @private
         */
        ...
        _showParticipantsForDate(dateStr, participants) {
            ...
            if (!Array.isArray(participants) || participants.length === 0) {
                ...
                }, isEnglish ? 'No participants available on this day.' : 'Nenhum participante disponível neste dia.');
        ...
        _showParticipantDetails(participant) {
            ...
            if (!participantInfo || !datesList) {
                console.warn('Participant details modal elements not found');
                return;
            }
            ...
            const cardBody = DOMUtils.createElement('div', {
                className: 'card-body'
            });
            
            // Nome com badge de admin
            const nameTitle = DOMUtils.createElement('h5', {
                className: 'card-title'
            }, participant.name || 'Nome indisponível');
            
            // [Continuação do método _showParticipantDetails]
            ...
            // Data de criação
            const createdText = DOMUtils.createElement('p', {
                className: 'card-text'
            });
        ...
        _showLoading(isLoading) {
            // Verificar se já existe um spinner
         ...
         * @returns {Object|null} Dados da survey ou null se não carregados
         */
        getSurveyData() {
        ...
        destroy() {
            ...
            // Nota: implementação específica dependeria dos listeners registrados
    ...
    // Exportar o módulo
    GroupSesh.Dashboard.Core = DashboardCore;
})();